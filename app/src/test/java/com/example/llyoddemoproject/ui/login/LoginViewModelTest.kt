...
import com.example.llyoddemoproject.ui.login.LoginViewModel
import com.example.llyoddemoproject.util.ENTER_EMAIL_ID
import com.example.llyoddemoproject.util.ENTER_PASSWORD
...
import com.example.llyoddemoproject.data.network.Resource as Resource1
...
class LoginViewModelTest : BaseViewModelRepositoryTest<LoginViewModel, LoginRepository>() {
    ...
    fun `onSignBtnClick, Data Error No network,assert showErrorDialog`() {
        ...
        coEvery {
            repository.login(any(), any())
        } returns flow { emit(testDataClassGenerator.getNoNetworkError() as Resource1<LoginResponse>) }
